...
.setColor("#2f3136")
.setAuthor({ name: `${message.guild.name} BAŞVURU SİSTEMİ`,iconURL: message.guild.iconURL({ dynamic: true }) })
.setDescription(`
...
`)
      ...
      .addComponents(
      new MessageButton().setCustomId("ytbaşvuruu").setLabel("Yt Başvuru").setStyle("SECONDARY"),
      new MessageButton().setCustomId("srmluluk").setLabel("Sorumluluk").setStyle("SECONDARY"),
     );
...
if(wsi.member.roles.cache.has(conf.başvuruendüşükyt)) return wsi.reply({embeds: [new Discord.MessageEmbed().setColor("#2f3136").setAuthor({ name: wsi.guild.name, iconURL: wsi.guild.iconURL({ dynamic: true }) }).setDescription(`Zaten Yetkili Rollerine Sahipsin Tekrardan Başvuru Yapamazsın`)], ephemeral: true })
 ...
 wsi.showModal(modal, { client, wsi });
    ...
    if(wsi.customId === 'ytbaşvuruform') {
      const aktiflikk = wsi.fields.getTextInputValue('aktiflikk');  
      const ytlikuğpdf = wsi.fields.getTextInputValue('ytlikuğpdf');  
      const swnelerypbl = wsi.fields.getTextInputValue('swnelerypbl');
      const kayıtprtn = wsi.fields.getTextInputValue('kayıtprtn');  
...
.setColor("RANDOM")
  ...
  **${wsi.user.tag}** - (\`${wsi.user.id}\`) **Kullanıcısının Sorumluluk Başvurusu Formu**
  ...
  .setTimestamp()
        ...
        let message = await client.channels.cache.get(conf.başvurulog).send({ content: `<@&${conf.başvurustaff}> ${wsi.user.toString()}`,  embeds: [ytalımformcevapbilg], components: [ytalımysnrrow]  })      
 ...
 wsi.showModal(modal2, { client, wsi });
  ...
  await wsi.message.delete().catch(e => {})
      const kbledşlmendf = wsi.fields.getTextInputValue('kbledşlmendf');

    let kblkişi = db.get(wsi.message.id)
        let kblkullanıcı = wsi.client.guilds.cache.get(conf.guildID).members.cache.get(kblkişi) 
...
Kabul Edilme Nedeni \`${kbledşlmendf}\`
...
await client.channels.cache.get(conf.başvurudurum).send({ content:`<@${kblkişi}>`,embeds: [kabullletttikkk]})
        db.delete(wsi.message.id)
         ...
         kblkullanıcı.roles.add(conf.başvuruytrolleri)
 ...
 if(!wsi.member.roles.cache.has(conf.başvurustaff)) return wsi.reply({ content: ":x: Sadece Yetkililer Red Edebilir.", ephemeral: true })
 ...
 wsi.showModal(modal3, { client, wsi });
  ...
  await wsi.message.delete().catch(e => {})
    const başvurudsfesf = wsi.fields.getTextInputValue('başvurudsfesf');

    let redkişi = db.get(wsi.message.id)
        let redkullanıcı = wsi.client.guilds.cache.get(conf.guildID).members.cache.get(redkişi) 
		const redetttikkk = new Discord.MessageEmbed()
.setColor("RANDOM")
.setDescription(`
> <@${redkişi}> Malesef Başvurun ${wsi.user.toString()} Tarafından Red Edildi

Red Edilme Nedeni \`${başvurudsfesf}\`
...
`);
  ...
  await client.channels.cache.get(conf.başvurudurum).send({ content:`<@${redkişi}>`,embeds: [redetttikkk]})
        redkullanıcı.send(`Malesef Yt Başvuru Talebin Red Edildi`);
  db.delete(wsi.message.id)
  await wsi.message.delete().catch(e => {})
