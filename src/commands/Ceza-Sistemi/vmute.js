...
const { green, Mute, revusome, kirmiziok } = require("../../configs/emojis.json")
module.exports = {
  conf: {
  ...
  run: async (client, message, args, embed) => {
    ...
    return }
    if (conf.voicemutelimit > 0 && vmuteLimit.has(message.author.id) && vmuteLimit.get(message.author.id) == conf.voicemutelimit) 
    {
    message.react("❌")
    ...
    if (conf.dmMessages) member.send({content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle **sesli kanallarda** susturuldunuz!`}).catch(() => {});
