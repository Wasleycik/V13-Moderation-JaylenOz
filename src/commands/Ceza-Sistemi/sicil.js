...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    data = data.map((x) => `#${x.id} ${x.active ? green : "❌"} **[${x.type}]** ${moment(x.date).format("LLL")} tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış.\n─────────────────`).join("\n");
