...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (conf.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == conf.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    message.react("✅")
    if (conf.dmMessages) user.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle banlandınız!`}).catch(() => {});
