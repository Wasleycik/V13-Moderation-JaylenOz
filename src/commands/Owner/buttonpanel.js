...
{
await interaction.reply({ content: `${member.toString()} Yapım Aşamasında`, ephemeral: true });
}

if(interaction.customId === "VI")
{
await interaction.reply({ content: `
\`••❯\` Sesli kanallardaki üye sayısı : \`${(interaction.guild.members.cache.filter((x) => x.voice.channel).size)}\`
