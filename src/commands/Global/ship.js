...
module.exports = {
 ...
 if(message.channel.id !== conf.shipchannel && message.channel.id !== conf.botcommandschannel) return message.reply({content: `Bu Komutu Sadece <#${conf.botcommandschannel}> Ve <#${conf.shipchannel}> Kanalında Kullanabilirsin.`}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
...
const applyText = (canvas, text) => {
      ...
      const background = await Canvas.loadImage(message.guild.bannerURL({ format: 'png' }));
...
if(random >= 90 && random < 95) shipdurum = "💖💖💖💖💖💖💖💖💖💔";
if(random >= 95 && random < 100) shipdurum = "💖💖💖💖💖💖💖💖💖💔";
if(random >= 100) shipdurum = "💖💝💖💝💖💝💖💝💖💝💝";
...
.setColor("RANDOM")
.setDescription(`
☁ **${nickname2} Bakalım ${nickname} Seni Çok Mu Seviyor?**
💟 ${random}%

${shipdurum}`)
.setImage("attachment://ozi.png")
message.channel.send({ content: `**[\`${member2.user.tag}\`] 🤔 [\`${member.user.tag}\`]**`, embeds: [ozi], files: [attachment] })
} else if (random <= 40) {
