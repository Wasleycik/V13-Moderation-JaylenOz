...
module.exports = {
    ...
    let taglilar = message.guild.members.cache.filter(s => s.user.username.includes(conf.tag) && !s.roles.cache.get(conf.ekipRolu))
...
Etkinlik/Çekiliş rolü olmayan ${ozi.size} kullanıcıya etkinlik, çekiliş rolleri verildi !`})
    ...
    if (button.customId === 'tagrol') {
      ...
      let taglilar = message.guild.members.cache.filter(s => s.user.username.includes(conf.tag) && !s.roles.cache.has(conf.ekipRolu))
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`})
    ...
    message.guild.members.cache.filter(s => s.user.username.includes(conf.tag) && !s.roles.cache.has(conf.ekipRolu)).map(x=> x.roles.add(conf.ekipRolu))                
    }
...
${ozicim.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `})
    ...
    message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0).map(x=> x.roles.add(conf.unregRoles))
