...
module.exports = async (message) => {
  ...
  if (data) {
    ...
    message.reply({ content:`Başarıyla Afk Modundan Çıkış Yaptınız. \`${moment.duration(Date.now() - afkData.date).format("d [gün] H [saat], m [dakika] s [saniye]")}\` süredir AFK'ydınız.`}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
  }
